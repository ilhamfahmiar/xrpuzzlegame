...
function recalculateLayout() {
  ...
  // Area holding: bagian kiri (35% lebar) — sesuai spec: holding area di sisi kiri
  const holdW = Math.floor((size - padding * 3) * HOLDING_AREA_RATIO);
  const holdH = size - padding * 2;
  const holdX = padding;
  const holdY = padding;

  // Area grid: bagian kanan (65% lebar) — sesuai spec: target area di sisi kanan
  const gridW = Math.floor((size - padding * 3) * (1 - HOLDING_AREA_RATIO));
  const gridH = gridW; // Persegi
  const gridX = holdX + holdW + padding;
